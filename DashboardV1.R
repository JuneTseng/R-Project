...
server <- function(input, output, session) {
  ...
  #-------
  dl<- df %>% group_by("MON000") 
  
  
  
  dh<- dl %>% select(report_week,power_outage_deaths_count, mortality_hospital_failure_cardiovascular_count) %>%
    ggplot(aes(report_week, mortality_hospital_failure_cardiovascular_count))+geom_jitter()
